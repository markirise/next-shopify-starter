...
export async function createCheckout(id, quantity, customAttributes) {
  // const ca = JSON.stringify(customAttributes);
  const sender = customAttributes["sender"]
  const recipient = customAttributes["recipient"];
  const message = customAttributes["message"];
...
export async function updateCheckout(id, lineItems) {  
  const formattedLineItems = lineItems.map(item => {
    ...
    console.log(JSON.stringify(item.customAttributes))
    
    const sender = item.customAttributes["sender"]
    const recipient = item.customAttributes["recipient"];
    const message = item.customAttributes["message"];
